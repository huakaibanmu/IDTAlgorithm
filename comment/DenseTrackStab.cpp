...
Video.cpp:         测试依赖库是否安装成功，与IDT算法无关
					
bound box相关内容
bound box提供视频帧中人体框的信息，
在计算前后帧的投影变换矩阵时，不使用人体框中的匹配点对，从而排除人体运动干扰，使得对相机运动的估计更加准确。
...
需要注意的是虽然要输入置信度，但实际上这个置信度在代码里也没有用上的样子，所以取任意值也不影响使用。
因为一帧图像可能框出来的人有好多个，
这种细粒度的控制比大致框出一个范围能更有效地滤去噪声.
至于如何获得这些bound box的数据，最暴力的方法当然是手工标注，不过这样太辛苦了。
在项目中我们采用了SSD（single shot multibox detector）/yolov3算法检测人体框的位置。
...
DenseTrackStab.h 定义了Track等的数据结构。最重要的track类里面可以看出：
	...
	int index;// 序号
基本方法就是在重采样中提取轨迹，在轨迹空间中再提取hog,hof,mbh特征，这些特征组合形成iDT特征，最终作为这个动作的描述。
